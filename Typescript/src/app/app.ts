 ...
 */
    ...
    container.documentEditor.documentName = 'Bookmark';
   ...
   bindData();
    ...
    function onselect(args: SelectEventArgs): void {
        ...
        insertField(fieldName,(args.data as any).id);
    }
    ...
    function insertField(fieldName: any,draggedItemId:any): void {
     
        let draggedText : any = fieldName.replace(/\n/g, '').replace(/\r/g, '').replace(/\r\n/g, '');
        container.documentEditor.editor.insertBookmark("draggedItem_" +draggedItemId);
        container.documentEditor.editor.insertText(draggedText);
        container.documentEditor.focusIn();
    }

    function getID(){

        let bookmarkCollection=container.documentEditor.getBookmarks();  
        for(let i=0;i<bookmarkCollection.length;i++){
            if(bookmarkCollection[i].startsWith("draggedItem_")){
                idCollection.push(bookmarkCollection[i]);
            }
        }
      }
     function bindData(){
        ...
        for(let i=0;i<idCollection.length;i++){
            replaceBookmarkContent(idCollection[i],"replaceText")
        }
      }
      function replaceBookmarkContent(bookmarkName:string, replaceText:string){
        container.documentEditor.selection.selectBookmark(bookmarkName);
        let startOffset=container.documentEditor.selection.startOffset;
        // Add your test to replace for specific ID
        container.documentEditor.editor.insertText(replaceText); 
        let currentOffset=container.documentEditor.selection.startOffset;
        container.documentEditor.selection.select(startOffset,currentOffset);
        container.documentEditor.editor.insertBookmark(bookmarkName);
        container.documentEditor.focusIn();
      }
